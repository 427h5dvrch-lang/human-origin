...
function buildPublicationJob({
  ...
  const job = {
    ...
    cartouche_png_path: cartouchePngPath || cartoucheCompactPngPath,

    certificate_json_path: certificateJsonPath || null,
    verify_txt_path: verifyTxtPath || null,
...
async function exportFinalProjectCertificate() {
  ...
  verifierUrl,
    ...
    try {
      ...
      const sendReadme = [
        "README — Vérifier ce document HumanOrigin",
        "",
        "Ce dossier contient les éléments nécessaires pour vérifier un document HumanOrigin.",
        "",
        "1. Ouvrez d’abord le PDF labellisé HumanOrigin.",
        `   ${canGeneratePublishedPdf ? sendPublishedPdfFilename : `${rawShareProjectName} — ${publishedDocumentFilename}`}`,
        "",
        "2. La marque HumanOrigin visible sur le document indique le niveau de preuve.",
        "",
        "3. Pour vérifier en ligne, ouvrez :",
        `   ${verifierUrl}`,
        "",
        "4. Dans le vérificateur, importez le fichier de preuve HumanOrigin contenu dans ce dossier.",
        `   ${sendProofFilename}`,
        "",
        "5. Vous pouvez aussi importer le PDF reçu pour confirmer qu’il correspond bien à la preuve.",
        "",
        "---",
        "",
        "README — Verify this HumanOrigin document",
        "",
        "This folder contains the files needed to verify a HumanOrigin document.",
        "",
        "1. First, open the HumanOrigin-labeled PDF.",
        `   ${canGeneratePublishedPdf ? sendPublishedPdfFilename : `${rawShareProjectName} — ${publishedDocumentFilename}`}`,
        "",
        "2. The visible HumanOrigin mark on the document shows the proof level.",
        "",
        "3. To verify online, open:",
        `   ${verifierUrl}`,
        "",
        "4. In the verifier, import the HumanOrigin proof file included in this folder.",
        `   ${sendProofFilename}`,
        "",
        "5. You may also import the received PDF to confirm that it matches the proof.",
        "",
        "---",
        "",
  ...
  on("export-success-copy-msg-btn", async () => {
    ...
    const msg = hoUiLang() === "en"
      ? [
          "Hello,",
          "",
          "I'm sending you my HumanOrigin-labeled document.",
          "",
          "This document contains a visible HumanOrigin mark: it indicates that an observed creation process has been linked to this file.",
          "",
          "To verify:",
          VERIFIER_URL,
          "",
          "You can open the PDF directly. For full verification, use the attached HumanOrigin proof kit.",
          "",
          "HumanOrigin verifies an observed creation process, not the content or truth of the document.",
          "",
          "Best regards,",
        ].join("\n")
      : [
          "Bonjour,",
          "",
          "Je vous transmets mon document labellisé HumanOrigin.",
          "",
          "Ce document porte une marque HumanOrigin visible : elle indique qu'un processus de création a été observé et lié à ce fichier.",
          "",
          "Pour vérifier :",
          VERIFIER_URL,
          "",
          "Vous pouvez ouvrir le PDF directement. Pour une vérification complète, utilisez le kit de preuve HumanOrigin joint.",
          "",
          "HumanOrigin vérifie un processus de création observé, pas le contenu ni la vérité du document.",
          "",
          "Bien à vous,",
        ].join("\n");
    try {
